...
void Start() {
    ...
    W2indow = glfwGetCurrentContext();
}

void Update() {
    float moveSpeed = 200.0f * deltaTime();
    
    if (glfwGetKey(W2indow, GLFW_KEY_W) == GLFW_PRESS)
    {
        holamundo.Position.y += moveSpeed;
    }
    if (glfwGetKey(W2indow, GLFW_KEY_S) == GLFW_PRESS)
    {
        holamundo.Position.y -= moveSpeed;
    }
    if (glfwGetKey(W2indow, GLFW_KEY_A) == GLFW_PRESS)
    {
        holamundo.Position.x -= moveSpeed;
    }
    if (glfwGetKey(W2indow, GLFW_KEY_D) == GLFW_PRESS)
    {
        holamundo.Position.x += moveSpeed;
    }
    
    if (glfwGetKey(W2indow, GLFW_KEY_SPACE) == GLFW_PRESS)
    {
